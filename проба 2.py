...
Я, на жаль, не допоможу із задачками про важелі, але можу запропонувати цілих два запитання із <b>фізики❄🔥</b> Яке тебе цікавить?''', reply_markup = physics_kb, parse_mode='html')
        ...
        bot.send_message(callback.message.chat.id, 'Чудово!')
        time.sleep(1)
        ...
        bot.send_message(callback.message.chat.id, '''«Дві речі наповнюють душу завжди новими і дедалі сильнішими подивом [...] — це зоряне небо наді мною і моральний закон у мені», - писав Іммануїл Кант. 
...
Що ж, <b>астрономія</b> - вкрай захоплива тема!🚀🪐 У ній я маю три запитання. Що тебе цікавить?😉''', reply_markup = astronomy_kb, parse_mode='html')    
        ...
        bot.send_message(callback.message.chat.id, '<b>Загальні запитання</b> - найцікавіша з усіх тем.🥳🥳 В цій темі є десять питань. Яка тобі до вподоби?😉', reply_markup = common_kb, parse_mode='html')

@bot.message_handler()
def questions(message):
    remove_kb = telebot.types.ReplyKeyboardRemove()
    if message.text == 'Обчислення потужності за законом Стефана-Больцмана':
        msg_physics = bot.send_message(message.chat.id, f'''Обрана тобою тема - <b>Обчислення потужності за законом Стефана-Больцмана</b>. Введи площу поверхні тіла та абсолютну температуру тіла без одиниць вимірювання через пробіл🤗''', parse_mode='html')
...
<b>h = 6.62607015×10^−34 Дж⋅с (Дж = кг⋅м^2 ⋅ с^−2)</b>''', parse_mode = 'html')
    elif message.text == 'Обчислення квадратного рівняння':
        msg_quadratic_equation = bot.send_message(message.chat.id, '<b>Квадратне рівняння?</b> Ізі!😎 Напиши значення параметрів а, b і c через пробіл', parse_mode='html')
        ...
        msg_dot_product = bot.send_message(message.chat.id, '<b>Скалярний добуток?</b> Та запросто!😁 Напиши мені значення координат а та b і кута θ між ними через пробіл і я все швидко порахую!', parse_mode='html')
        ...
        msg_distance = bot.send_message(message.chat.id, '<b>Відстань від точки до прямої в просторі...</b> Гм, не таке вже й легке запитання🤔 Напиши мені координати точки в просторі і координати точок а та b на цій прямій через пробіл без дужок. Спробую щось придумати', parse_mode='html')
        bot.register_next_step_handler(msg_distance, func_distance)
    elif message.text == 'Обчислення площі трикутника':
        msg_triangle = bot.send_message(message.chat.id, '<b>Площа трикутника?</b> Це мені до снаги!🤟 Напиши довжину основи трикутника і довжину висоти, опущеної на цю основу, без пробілів та одиниць вимірювання, і я скажу тобі площу твого трикутника🤗')
        bot.register_next_step_handler(msg_triangle, func_triangle)
    elif message.text == 'Виведення числа π':
        bot.send_message(message.chat.id, 'Число π (пі) для програмування та використання при складних і точних обчисленнях комп‘ютером — <b>3,14159265358979.</b>', parse_mode='html')
    elif message.text == '5 найвищих гір світу':
        bot.send_photo(message.chat.id, 'https://t3.gstatic.com/licensed-image?q=tbn:ANd9GcRh6kEzhH9PktdhztbPzjD6ohLYRKJZx0rQ3PRYFfWMbu-FG4WSm79ntCS7_U20tPkK')
...
Росія також має 14 сусідів. Сусідніми державами Росії є: Китай, Україна, Білорусь, Польща, Північна Корея, Казахстан, Фінляндія, Грузія, Норвегія, Монголія, Литва, Азербайджан, Естонія та Латвія.''', parse_mode='html')
    elif message.text == 'Найбільш населене місто світу':
        bot.send_photo(message.chat.id, 'https://planetofhotels.com/guide/sites/default/files/styles/big_gallery_image/public/text_gallery/Shanghai-3.jpg')
...
На душу населення – 30,5 тис. куб. м''', parse_mode='html')
    elif message.text == 'Відстань між Землею та Сонцем':
        bot.send_message(message.chat.id, '''Середня відстань від Землі до Сонця становить приблизно <b>149,6 млн. км.</b> Більш точне число - 149 597 870 700 м, воно також називається астрономічною одиницею.''', parse_mode='html')
...
<b>Комета</b> - мале тіло Сонячної системи, яке обертається навколо Сонця і має так звану кому та/або хвіст.''', parse_mode='html')    
    elif message.text == 'Процеси, що відбуваються на Сонці, та їхній вплив на Землю':
        bot.send_photo(message.chat.id, 'https://4.bp.blogspot.com/-NOhm0yRSVRc/UOluz84WIYI/AAAAAAAAEEc/Cuvk7WEEZkA/s1600/%D1%81%D0%BE%D0%BD%D1%86%D0%B5+%D0%B0%D0%BA%D1%82%D0%B8%D0%B2%D0%BD%D0%B5.bmp')
...
У людей з хворобами серцево-судинної системи під час геомагнітних бур, спричинених Сонцем, погіршується стан, збільшується число інфарктів та інсультів. У здорових людей змінюється сприйняття часу, сповільнюється рухова реакція, знижується короткочасна пам'ять, об'єм та інтенсивність уваги. Навіть у спеціально тренованих людей - спортсменів вищого класу та льотчиків — зафіксовано підвищену кількість помилок при виконанні контрольних завдань.
 ''', parse_mode='html')
        ...
        bot.send_message(message.chat.id, f'Зараз <b>{mess_year} рік</b>', parse_mode='html')
    elif message.text == 'Скільки днів у ___ році':
        msg_days_in_year = bot.send_message(message.chat.id, 'Введи рік числом без слів')
    ...
    elif message.text == 'Пограти у вгадай число між 1 та 10':
        msg_game = bot.send_message(message.chat.id, 'Введи твоє число')
...
– Повертатися – погана прикмета.''', 'Штірліц довго дивився на одну крапку. Потім перевів погляд на иншу крапку. “Двокрапка” — здогадався Штірліц.', 'Штирліц сидів дома. З вікна дуло. Штирліц натиснув Alt+F4 — вікно зникло.', 'Вечорами Мальвіна любила дивитися одним оком на зоряне небо та згадувати той незабутній поцілунок Буратіно.']
        ...
        bot.send_photo(message.chat.id, 'https://img.freepik.com/premium-vector/rich-wealthy-happy-smiling-businessman-worker-entrepreneur-character-sitting-pile_133260-2079.jpg')
...
Відомий як «Оракул з Омахи», Воррен Баффетт є одним із найуспішніших інвесторів усіх часів. Він управляє інвестиційним конгломератом Berkshire Hathaway, до якого входять десятки компаній, серед яких страховик Geico, виробник батарейок Duracell і мережа ресторанів Dairy Queen. Син американського конгресмена, він уперше купив акції в 11 років, а в 13 уперше подав податкову декларацію.''', parse_mode='html')
        ...
        bot.send_photo(message.chat.id, 'https://img.tsn.ua/cached/115/tsn-d7721072f5f1787fa1f16a6df613a75f/thumbs/1036x648/b6/5a/423e19327435eaeb23b71f8a12905ab6.jpeg')
...
''']
        ...
        if 'Мій пес' in fav_movie: 
            bot.send_photo(message.chat.id, 'https://planetakino.ua/res/get-poster/00000000000000000000000000002505/1600-1040_MyStupidDog.jpg')
            bot.send_message(message.chat.id, fav_movie, '<i>[Інформацію взято з сайту 24tv.ua]</i>', parse_mode='html')
        elif 'Дівчина мого найкращого друга' in fav_movie:
            bot.send_photo(message.chat.id, 'https://m.media-amazon.com/images/I/51YSoCnhxgL.jpg')
            bot.send_message(message.chat.id, fav_movie, '<i>[Інформацію взято з сайту 24tv.ua]</i>', parse_mode='html')
        elif 'Віолет і Дейзі' in fav_movie:
            bot.send_photo(message.chat.id, 'https://www.film.ru/sites/default/files/styles/epsa_685x385/public/trailers_frame/f_110988.jpg') 
            bot.send_message(message.chat.id, fav_movie, '<i>[Інформацію взято з сайту 24tv.ua]</i>', parse_mode='html')
        elif 'Астерікс на Олімпійських іграх' in fav_movie:
            bot.send_photo(message.chat.id, 'https://s6.vcdn.biz/static/f/1414570551/image.jpg/pt/r300x423x4') 
            bot.send_message(message.chat.id, fav_movie, '<i>[Інформацію взято з сайту 24tv.ua]</i>', parse_mode='html')
        else:
            bot.send_photo(message.chat.id, 'https://ictv.ua/wp-content/uploads/2016/12/16/Pohmillya-u-Vegasi-1.jpg') 
            bot.send_message(message.chat.id, fav_movie, '<i>[Інформацію взято з сайту 24tv.ua]</i>', parse_mode='html')
...
x2 = {x2}''')
        ...
        bot.send_message(message.chat.id, 'Рівняння немає коренів')
...
@bot.message_handler()
def physics(message):
    if message.text == 'Обчислення квадратного рівняння':
        bot.send_message(message.chat.id, 'Ти довбойоб сам порішать не можеш??? Сука як ви всі заїбали')
...
def puk(message):
    bot.send_message(message.chat.id, '''Ви обрали питання Обчислення потужності за законом Стефана-Больцмана. Введіть площу поверхні тіла та абсолютну температуру тіла без одиниць вимірювання через пробіл''')    


topics_questions = ['обчислення потужності за законом стефана-больцмана', 'виведення сталої планка']

@bot.message_handler(func = lambda message: message.text.lower() == 'фізика')

def physics(message):
    bot.send_message(message.chat.id, f'''Ви обрали тему {message.text.lower()}. В цій темі я можу допомогти із такими питаннями:
                     
Обчислення потужності за законом Стефана-Больцмана;
Виведення сталої Планка.
    
Напиши, яка тема тобі потрібна❤️''')

@bot.message_handler(func = lambda message: message.text.lower() == 'обчислення потужності за законом стефана-больцмана')

def stefan_boltzmann_law(message):
     msg = bot.send_message(message.chat.id, f'''Ви обрали питання Обчислення потужності за законом Стефана-Больцмана. Введіть площу поверхні тіла та абсолютну температуру тіла без одиниць вимірювання через пробіл''')
     bot.register_next_step_handler(msg, caltulate_answer)

def caltulate_answer(message):
     a, t = message.text.split()
     A = int(a)
     T = int(t)
     P = 5,67 * A * (T**4)
     bot.send_message(message.chat.id, f'Потужність випромінювання: Р = {P}')

bot.polling(none_stop = True, interval = 0)


def get_user_text(message):
    if message.text.lower() == 'фізика':
        physics
    elif message.text.lower() == topics[1]:
        bot.send_message(message.chat.id, f'Ви обрали тему {topics[1]}')
    elif message.text.lower() == topics[2]:
        bot.send_message(message.chat.id, f'Ви обрали тему {topics[2]}')
    elif message.text.lower() == topics[3]:
        bot.send_message(message.chat.id, f'Ви обрали тему {topics[3]}')
    else:
        bot.send_message(message.chat.id, 'Я не знаю цієї теми☹ Ви точно обрали тему зі списку?')
